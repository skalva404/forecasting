...
import com.forecasting.models.MultiSeasonalForecaster;
...
public class BenchmarkWeeklyForecastTest extends TestCase {


    private static Logger logger = Logger.getLogger(BenchmarkWeeklyForecastTest.class.getName());
    private static final String DELIM1 = ",";
    private static final String DELIM2 = "|";
    private static final int BATCH_SIZE = 200;


    public BenchmarkWeeklyForecastTest(String testName) {
        super(testName);
        try {
    ...
    public static Test suite() {
        return new TestSuite(BenchmarkWeeklyForecastTest.class);
    }
    ...
    public void testGoldenDataSet() {
        try {
            ...
            // Forecast parameters
            int trainPoints = 65;
            int seasonalPeriod1 = 4;
            int seasonalPeriod2 = 13;
            int validationPoints = 13;
            int futurePoints = 13;
            List<Model> models = new ArrayList<Model>();
            models.add(Model.MSTESA);
            models.add(Model.MSTESM);
            models.add(Model.ENSEMBLE);

            MultiSeasonalForecaster forecaster = new MultiSeasonalForecaster();
            ...
            for (String key : timeSeriesMap.keySet()) {
                ...
                DataSet forecastTimeSeries = forecaster.forecast(trainPoints, validationPoints, futurePoints, seasonalPeriod1, seasonalPeriod2);
                forecastTimeSeriesMap.put(key, forecastTimeSeries);

                if (forecastTimeSeriesMap.size() % 25 == 0) {
                    prog = forecastTimeSeriesMap.size() * 100 / timeSeriesMap.size();
                    System.out.println("Progress = " + prog);
                }
            }


            System.out.println("forecastTimeSeriesMap = " + forecastTimeSeriesMap.size());
            System.out.println("timeSeriesMap = " + timeSeriesMap.size());
    ...
    private void writeMetricsToFile(Map<String, DataSet> dataSetMap) {
        ...
        try {
            ...
            String fileName = this.getClass().getResource(ConfigManager.get("metrics.weekly")).getPath();
    ...
    private Map<String, double[]> loadActualData() {
        try {
            ...
            int testDataSize = 13;
            String fileName = this.getClass().getResource(ConfigManager.get("testDataSet.weekly")).getPath();
    ...
    private void writeToFile(Map<String, DataSet> dataSetMap) {
        ...
        try {
            String fileName = this.getClass().getResource(ConfigManager.get("output.weekly")).getPath();
            BufferedWriter fileWriter = new BufferedWriter(new FileWriter(new File(fileName)));
