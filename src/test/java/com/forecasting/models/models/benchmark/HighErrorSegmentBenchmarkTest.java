...
public class HighErrorSegmentBenchmarkTest extends TestCase {


    private static Logger logger = Logger.getLogger(HighErrorSegmentBenchmarkTest.class.getName());
    private static final String DELIM1 = ",";
    private static final String DELIM2 = "|";
    private static final int BATCH_SIZE = 200;


    public HighErrorSegmentBenchmarkTest(String testName) {
        super(testName);
        try {
    ...
    public static Test suite() {
        return new TestSuite(HighErrorSegmentBenchmarkTest.class);
    }
    ...
    public void testGoldenDataSet() {
        try {
            ...
            String strLine;
            String fileName = this.getClass().getResource(ConfigManager.get("input.higherror")).getPath();
            ...
            while ((strLine = br.readLine()) != null) {
                ...
                if (tsList.size() == 0 || tsList.contains(token[0])) {
            ...
            if (writeDebugFile)
                createDebugFile(timeSeriesMap, forecastTimeSeriesMap);
    ...
    private void createDebugFile(Map<String, DataSet> timeSeriesMap, Map<String, DataSet> forecastTimeSeriesMap) {

        try {

            String fileName = ConfigManager.get("debugFile");
            ...
            for (String key : timeSeriesMap.keySet()) {
                DataSet ds = timeSeriesMap.get(key);
                double[] actual = actualMap.get(key);
                double[] forecast = forecastTimeSeriesMap.get(key).toArray();
                i = 0;

                fileWriter.write("\n\n" + key + "\n");
                for (DataPoint dp : ds.getDataPoints()) {
                    {
                        line = ++i + "," + dp.getDependentValue() + "," + dp.getDependentValue() + "\n";
                ...
                for (int j = 0; j < actual.length; j++) {
                    line = ++i + "," + actual[j] + "," + (long) forecast[j] + "\n";
                    fileWriter.write(line);
                }
                fileWriter.flush();
            }
    ...
    private void writeMetricsToFile(Map<String, DataSet> dataSetMap) {
        ...
        try {
            ...
            double[] actual_req = new double[testPoints];
            double[] forecast_req = new double[testPoints];
            double boundSum = 0;
            ...
            for (String key : dataSetMap.keySet()) {
                if (null == actualMap.get(key))
                    ...
                    for (DataPoint dp : dataPoints) {
                        ...
                        actual_req[i] += actual[i];
                        forecast_req[i] += dp.getDependentValue();
            ...
            for (int i = 0; i < testPoints; i++) {
                ...
                sb.append("%\n");
                sb.append(" Actual Requests Sum          = " + nf.format(actual_req[i]));
                sb.append("%\n");
                sb.append(" Forecast Requests Sum        = " + nf.format(forecast_req[i]));
                sb.append("%\n");
                sb.append(" Overall Avg Error            = " + nf.format(Math.abs(forecast_req[i] - actual_req[i]) * 100 / actual_req[i]));
                sb.append("%\n");
...
//                sb.append(" Bound Sum      = " + nf.format(boundSum) + "%");
            ...
            String fileName = this.getClass().getResource(ConfigManager.get("metrics.higherror")).getPath();
    ...
    private Map<String, double[]> loadActualData() {
        try {
            ...
            String fileName = this.getClass().getResource(ConfigManager.get("testDataSet.higherror")).getPath();
    ...
    private void writeToFile(Map<String, DataSet> dataSetMap) {
        ...
        try {
            String fileName = this.getClass().getResource(ConfigManager.get("output.higherror")).getPath();
