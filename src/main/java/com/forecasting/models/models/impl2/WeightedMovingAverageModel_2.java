...
@Data
public class
WeightedMovingAverageModel_2 extends AbstractForecastModel {
     ...
     */
    public WeightedMovingAverageModel_2(float[] weights, int slices, int trainPoints, int validationPoints) {
     ...
     * @param validationPoints
     */
    public WeightedMovingAverageModel_2(float[] weights, int slices, int step, int trainPoints, int validationPoints) {
    ...
    public void train() {
        ...
        for (int pos = trainPoints, j = 0; pos < trainPoints + validationPoints; pos++) {
            sum = 0;
            w = 0;
            for (int k = pos - step * slices; k < pos; k += step)
                sum += weights[w++] * (k < trainPoints ? actual[k] : valMatrix[k - trainPoints][1]);
            forecast = sum / slices;
            valMatrix[pos - trainPoints][0] = actual[pos];
            valMatrix[pos - trainPoints][1] = forecast;
        }
